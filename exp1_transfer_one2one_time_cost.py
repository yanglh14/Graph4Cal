# script to cal. time cost of CafkNet(one2one)
# (:: Graph4Cal\exp1_transfer_one2one_for_plot.py)
# 
# Z. Zhang
# 01/2024
...
def train(train_cables=4, eval_cables=5):
    ...
    for data in val_loader:
        ...
        out = model(data.x, data.edge_index, data.edge_features)
        endT = datetime.now()
        print("Time used:", endT - startT)

        # loss = torch.sqrt(loss_fn(out, data.y.view(-1, 3)))
        # print(loss)
        break

    ## plot one2one results
...
if __name__ == '__main__':
    ...
    for i in range(4, 11):
        ...
        j = i
        print('----------------------------------')
        print(f'Train on {i} cables, Test on {j} cables')
        test_loss = train(train_cables=i,eval_cables=j)
        test_loss_list[i,j] = test_loss
