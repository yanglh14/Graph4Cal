...
class GraphLayer(MessagePassing):
    ...
    def forward(self, h, edge_index, edge_features):
        h = self.propagate(edge_index[2:4], h=h, edge_type = 0, edge_features = edge_features[1::2].reshape(-1,3))
        h = self.propagate(edge_index[0:2], h=h, edge_type = 1, edge_features = edge_features[::2].reshape(-1,3))
...
class GraphNet(nn.Module):
    ...
    def forward(self, h, edge_index, edge_features):
        ...
        o = self.regression(h)
        o = o.view(-1, self.num_cables+2)[:,1:-1]
        return o
